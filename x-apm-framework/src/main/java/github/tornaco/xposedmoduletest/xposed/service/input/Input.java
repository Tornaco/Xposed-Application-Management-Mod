...
public class Input {
     ...
     * Run command in single thread executor.
     *
     * @param args The command-line arguments
     */
    public static void main(String[] args) {
        EXECUTOR.execute(() -> (new Input()).run(args));
    }

    private void run(String[] args) {
        Log.d(XposedLog.TAG + TAG, "run: " + Arrays.toString(args));
        if (args.length < 1) {
            showUsage();
            return;
        }
        ...
        try {
            if (command.equals("text")) {
                ...
                Log.d(XposedLog.TAG + TAG, "Error: Unknown command: " + command);
                showUsage();
                return;
            }
        } catch (NumberFormatException ex) {
        }
        Log.d(XposedLog.TAG + TAG, INVALID_ARGUMENTS + command);
        showUsage();
    }
    ...
    private void injectKeyEvent(KeyEvent event) {
        Log.i(XposedLog.TAG + TAG, "injectKeyEvent: " + event);
    ...
    private void injectMotionEvent(int inputSource, int action, long when, float x, float y, float pressure) {
        ...
        Log.i(XposedLog.TAG + TAG, "injectMotionEvent: " + event);
        InputManager.getInstance().injectInputEvent(event,
                InputManager.INJECT_INPUT_EVENT_MODE_WAIT_FOR_FINISH);
    }
    ...
    private void showUsage() {
        Log.d(XposedLog.TAG + TAG, "Usage: input [<source>] <command> [<arg>...]");
        Log.d(XposedLog.TAG + TAG, "");
        Log.d(XposedLog.TAG + TAG, "The sources are: ");
        for (String src : SOURCES.keySet()) {
            Log.d(XposedLog.TAG + TAG, "      " + src);
        }
        Log.d(XposedLog.TAG + TAG, "");
        Log.d(XposedLog.TAG + TAG, "The commands and default sources are:");
        Log.d(XposedLog.TAG + TAG, "      text <string> (Default: touchscreen)");
        Log.d(XposedLog.TAG + TAG, "      keyevent [--longpress] <key code number or name> ..."
                + " (Default: keyboard)");
        Log.d(XposedLog.TAG + TAG, "      tap <x> <y> (Default: touchscreen)");
        Log.d(XposedLog.TAG + TAG, "      swipe <x1> <y1> <x2> <y2> [duration(ms)]"
                + " (Default: touchscreen)");
        Log.d(XposedLog.TAG + TAG, "      draganddrop <x1> <y1> <x2> <y2> [duration(ms)]"
                + " (Default: touchscreen)");
        Log.d(XposedLog.TAG + TAG, "      press (Default: trackball)");
        Log.d(XposedLog.TAG + TAG, "      roll <dx> <dy> (Default: trackball)");
    }
}
